...
if( is_array($language_id) ){
	$language_id = ( isset($language_id[1]) ? $language_id[1] : $language_id );
}
...
$code .= '<li id="element'.$suranid.'" onClick="javascript:changeText('.$suranid.', \''.str_replace("&apos;","`", $surhname).'\', \''.$arrayname[$i][1].'\', \''.$img.'\')">'.$suranid.'- '.$surhname.'</li>'.$nl;
}
$code .= '</ul>'.$nl;
$code .= '</div>'.$nl;
}
...
function quran_playlist_content_replace($t){
	$text = preg_replace_callback("/quran_playlist\[([0-9]*?)\]/s", "QuranTranslations_sounds_loop", $t);
	return $text;
}
